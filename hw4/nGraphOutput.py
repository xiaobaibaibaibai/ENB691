...
def groupTrain():
    ...
    for i in [2, 8, 16]:
        # Select device
        deviceType = "/cpu:0"  
        tfConfig = tf.ConfigProto(intra_op_parallelism_threads=16, inter_op_parallelism_threads=i, allow_soft_placement=True, device_count = {'CPU': 10})
        tfConfig.gpu_options.allow_growth = True

        print("\n\n--------------------------------------------")
        print("-----------intra is {0}, inter is {1}-----------".format(16, i))
        print("--------------------------------------------")
        ...
        def complexModel():
            with tf.device(deviceType):
                ...
                hPooling = tf.nn.max_pool(value=h, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
                # ((13-2)/2)+1 = 6
